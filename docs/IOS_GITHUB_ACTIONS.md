...
| `IONCameraLib does not support provisioning profiles` | Nao passar `PROVISIONING_PROFILE` no `xcodebuild`. O CI aplica o perfil so no target App (`scripts/apply-ios-app-target-signing.js`) |
